...
{	protected:
	...
	public:
	aabb *left, *right;
	aabb(): min(vec(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),std::numeric_limits<float>::max())), max(vec(-std::numeric_limits<float>::max(),-std::numeric_limits<float>::max(),-std::numeric_limits<float>::max())),left(NULL),right(NULL),id(-1) {}
	aabb(const vec& x,const vec& y): min(x), max(y), left(NULL), right(NULL), id(-1) {}
	vec get_min() const
	{	return min;		}
	vec get_max() const
	{	return max;		}
	...
	{	if(a.size()==1) 
		...
		{	for(i=0;i<l.size();i++)
			b = b.group(b,l[i]);
			left = new aabb(b.get_min(),b.get_max());
		}

		if(r.size())
		{	for(i=0;i<r.size();i++)
			n = n.group(n,r[i]);
			right = new aabb(n.get_min(),n.get_max());
		}
	...
	void traverse(ray &r, std::vector<shape*>& a, std::queue<aabb*>& q)
	{	while(!q.empty())
		{	aabb *x = q.front();
			q.pop();
			if(x->intersect(r))
