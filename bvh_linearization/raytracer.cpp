...
{	
	...
	loader.Load_Model(shapes, world_boundingbox, box_stack, id1);
	bvh_node root = bvh_node(world_boundingbox,0);
	root.split(world_boundingbox,box_stack, ids, id1, nodes, 0);	//Building the aabb
	bvh master = bvh(nodes);	
	for(i=0;i<nodes.size();i++)
	{	if(nodes[i].size) printf("off=%d\n",nodes[i].offset.off);
		else printf("roff=%d\n",nodes[i].offset.roff);
	}
	clock_gettime(CLOCK_MONOTONIC, &t2);	//calculating time for the above process
	printf("SPLIT DONE! - Final added prims = %lu and nodes size=%lu\n",ids.size(),nodes.size());
	...
	{	for(j=0;j<width;j++)
		{	float _x = x_init + (unit/width) * j;
			...
			std::stack<int> st2;
			st.push(master.arr[0]);
			st2.push(0);
			master.traverse(r, shapes, ids, st, st2);	//BVH traversal to find object intersecting with ray
			int rid = r.get_rayid();
			if(rid != -1)
